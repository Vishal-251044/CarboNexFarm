...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    if (!loggedUser) {
      ...
      fetch(`http://localhost:5000/api/farmer/${loggedUser.email}`)
        .then((response) => {
          if (response.ok) {
        ...
        .catch(() => {
          fetch(`http://localhost:5000/api/company/${loggedUser.email}`)
            .then((response) => {
              if (response.ok) {
                return response.json();
              }
  ...
  const handleGetCarbonCredit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/api/predict-carbon-points",
        {
          method: "POST",
          headers: {
      ...
      const updateResponse = await fetch(
        "http://localhost:5000/api/farmer/update-carbon-points",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSellCarbonCredit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/sell-carbon", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
